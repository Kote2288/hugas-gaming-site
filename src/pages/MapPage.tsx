...
const MAP_IMG =
  "https://cdn.poehali.dev/projects/a6cbd089-012a-4d63-aa2c-4ebf4b7be9ea/files/c12d881a-afb2-46ed-aec8-21d608d4be66.jpg";
...
const LOCATIONS = [
  {
    ...
    items: [
      "Автоматы и снайперские винтовки",
      "Военные разгрузки и каски",
      "Рации и патроны",
      "Медпакеты NATO",
      "Военные консервы",
    ],
  ...
  },
  {
    ...
    label: "Деревни",
    icon: "🏡",
    color: "text-orange-400 border-orange-400/30 bg-orange-900/10",
    items: [
      "Охотничьи ружья",
      "Фермерская одежда",
      "Еда и семена",
  ...
  },
  {
    ...
    label: "Леса",
    icon: "🌲",
    color: "text-emerald-400 border-emerald-400/30 bg-emerald-900/10",
    items: [
      "Ягоды и грибы",
      "Охота на животных",
      "Материалы для костра",
      "Ловушки",
      "Маскировочные сети",
    ],
  },
  ...
  },
  {
    ...
    label: "Побережье",
    icon: "⚓",
    color: "text-cyan-400 border-cyan-400/30 bg-cyan-900/10",
    items: [
      "Рыбацкое снаряжение",
      "Лодки",
      "Навигация",
...
const KNOWN_TOWNS = [
  "Чернорудск (Chernogorsk)",
  "Электрозаводск (Elektrozavodsk)",
  "Балота (Balota)",
  "Солничный (Solnichniy)",
  "Бор (Bor)",
  "Бережки (Berezki)",
    ...
    <div className="min-h-screen bg-dark-bg pt-20 pb-16">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="py-8 border-b border-neon/20 mb-8">
          ...
          <h1 className="font-oswald text-4xl text-white">КАРТА LIVONIA</h1>
          <p className="text-gray-500 text-sm mt-2">
            Исследуй каждый уголок пост-апокалиптической Черноруссии
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <div className="space-y-2">
              {LOCATIONS.map((loc) => (
                <button
                  key={loc.id}
                  onClick={() =>
                    setActiveCategory(activeCategory === loc.id ? null : loc.id)
                  }
                  className={`w-full text-left border px-4 py-3 flex items-center justify-between transition-all ${
                    activeCategory === loc.id
                      ? loc.color
                      : "border-neon/10 bg-dark-card text-gray-400 hover:border-neon/30"
                  }`}
                >
                  ...
                  <Icon
                    name={
                      activeCategory === loc.id ? "ChevronUp" : "ChevronDown"
                    }
                    size={14}
                  />
                </button>
            ...
            {active && (
              <div
                className={`mt-3 border p-4 animate-fade-in ${active.color}`}
              >
                <div className="font-mono-tech text-xs tracking-widest mb-3 opacity-70">
                  // ПРЕДМЕТЫ
                </div>
            ...
            <div className="mt-6 border border-red-500/20 p-4 bg-red-900/5">
              <div className="font-mono-tech text-xs text-red-400 tracking-widest mb-3">
                // ОПАСНЫЕ ЗОНЫ
              </div>
              {[
                ...
                >
                  ...
                  <span
                    className={`font-mono-tech text-xs ${
                      z.danger === "СМЕРТЕЛЬНАЯ"
                        ? "text-red-500"
                        : z.danger === "ЭКСТРЕМАЛЬНАЯ"
                          ? "text-red-400"
                          : "text-orange-400"
                    }`}
                  >
