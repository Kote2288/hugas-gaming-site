    ...
    <div className="card-dark flex flex-col hover:border-neon/40 transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between">
          <div>
            {product.discount && (
              <div className="font-mono-tech text-xs text-gray-600 line-through">
                {product.price}₽
              </div>
            )}
            <div className="font-oswald text-2xl text-neon font-bold">
              {discountedPrice}₽
            </div>
          </div>
          <button
...
export default function Shop() {
  const { user, cart, products, purchases } = useStore() as ReturnType<
    typeof useStore
  > & { purchases?: unknown[] };
  ...
  const discountedTotal = promoDiscount
    ? Math.round(rawTotal * (1 - promoDiscount / 100))
    : rawTotal;
  ...
  return (
    <div className="min-h-screen bg-dark-bg pt-20 pb-16">
            ...
            <div className="flex items-center gap-3">
              {user && (
                <div className="flex items-center gap-2 border border-neon/30 px-4 py-2">
                  ...
                  <span className="font-oswald text-lg text-neon">
                    {user.balance}₽
                  </span>
                  <button
                    onClick={() => setShowTopUp(true)}
                    className="ml-2 btn-neon px-2 py-1 text-xs"
                  >
              ...
              <button
                ...
                className="btn-neon px-4 py-2 flex items-center gap-2 relative"
              >
                <Icon name="ShoppingCart" size={16} />
                КОРЗИНА
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2 w-5 h-5 bg-neon text-dark-bg text-xs font-bold rounded-full flex items-center justify-center">
                    {cartCount}
                  </span>
                )}
              </button>
                ...
                user.purchases.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span
                        className={`font-mono-tech text-xs px-2 py-1 border ${p.status === "delivered" ? "border-green-500/30 text-green-400 bg-green-900/10" : "border-yellow-500/30 text-yellow-400 bg-yellow-900/10"}`}
                      >
        ...
        {tab === "shop" && (
          <div className="flex gap-6">
            ...
            <div className="w-48 shrink-0 hidden md:block">
              <div className="font-mono-tech text-xs text-gray-600 tracking-widest mb-3">
                КАТЕГОРИИ
              </div>
                ...
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 flex items-center gap-2 text-sm transition-all ${
                      category === cat.id
                        ? "bg-neon/10 border-l-2 border-neon text-neon font-oswald"
      ...
      {showCart && (
        ...
        >
          <div
            className="bg-dark-card border border-neon/30 w-full max-w-lg max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="p-6">
              {cart.length === 0 ? (
                <p className="text-gray-500 text-center py-8 font-oswald">
                  Корзина пуста
                </p>
              ) : (
                <>
                  <div className="space-y-3 mb-6">
                    {cart.map((item) => {
                      ...
                      return (
                        <div
                          key={item.product.id}
                          className="flex items-center gap-3 border border-neon/10 p-3"
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <div className="font-mono-tech text-neon text-xs">
                              {price}₽
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="w-6 h-6 border border-neon/30 text-neon flex items-center justify-center hover:bg-neon/10"
                            >
                              <Icon name="Minus" size={10} />
                            </button>
                            <span className="font-mono-tech text-sm w-4 text-center">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() =>
                  ...
                  {promoMsg && (
                    <div
                      className={`font-mono-tech text-xs mb-4 ${promoMsg.startsWith("✓") ? "text-green-400" : "text-red-400"}`}
                    >
                      {promoMsg}
                    </div>
                  )}
                  ...
                  <div className="border-t border-neon/20 pt-4">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-oswald text-xl text-neon">
                        {discountedTotal}₽
                      </span>
                    </div>
                    {user && (
                      <div className="flex justify-between mb-4">
                        ...
                        <span
                          className={`font-mono-tech text-xs ${user.balance >= discountedTotal ? "text-green-400" : "text-red-400"}`}
                        >
                          {user.balance}₽
                        </span>
                      </div>
                    ...
                    <button
                      onClick={handleBuy}
                      disabled={!user}
                      className="w-full btn-neon-filled py-3 text-sm flex items-center justify-center gap-2"
                    >
                      <Icon name="CreditCard" size={16} />
                      ОПЛАТИТЬ {discountedTotal}₽
                    </button>
                    {!user && (
                      <p className="text-red-400 text-xs text-center mt-2 font-mono-tech">
                        Войдите через Steam
                      </p>
                    )}
                  </div>
      ...
      {showTopUp && (
        ...
        >
          <div
            className="bg-dark-card border border-neon/30 w-full max-w-md p-6"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-gray-600 text-xs font-mono-tech mb-4">
              * Оплата через ЮКасса, СБП и банковские карты. Баланс пополняется
              мгновенно.
            </p>
            <button
              onClick={() => {
      ...
      {showGuide && (
        ...
        >
          <div
            className="bg-dark-card border border-neon/30 w-full max-w-lg p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-oswald text-xl text-white">
            ...
            <div className="space-y-4">
              {[
                {
                  step: "01",
                  title: "Купите товар в магазине",
                  desc: "Выберите товар, добавьте в корзину и оплатите с баланса.",
                },
                {
                  step: "02",
                  title: "Зайдите на сервер",
                  desc: "Подключитесь к Odium Server через DayZ по IP play.odium.ru:2302",
                },
                {
                  step: "03",
                  title: "Введите команду",
                  desc: "В игровом чате напишите /shop — откроется список ваших покупок.",
                },
                {
                  step: "04",
                  title: "Получите предмет",
                  desc: 'Нажмите "Получить" рядом с купленным товаром. Предмет появится в вашем инвентаре.',
                },
              ].map((s) => (
                <div
                  key={s.step}
                  className="flex gap-4 border-b border-neon/10 pb-4"
                >
                  <div className="font-mono-tech text-neon text-lg font-bold shrink-0">
                    {s.step}
                  </div>
                  <div>
